...
inline void mapAdd(map<long,long double > &m, long key, long double val){//{{{
   if(m.count(key)==0)
      m[key] = val;
   else
      m[key] += val;
...
bool ReadDistribution::init(TranscriptInfo* trI, TranscriptSequence* trS, TranscriptExpression* trE, bool verb){ //{{{
   ...
   weightNorms.resize(3,vector<map<long, long double> >(M));
   // Initialize position bias matrices:
   posProb.resize( 6, vector<vector<long double> >(trSizesN + 1, vector<long double>(trNumberOfBins,0.01/trNumberOfBins)));
...
void ReadDistribution::setLength(long double mu, long double sigma){ //{{{
   lMu=mu;
   lSigma=sigma;
   lengthSet=true;
}//}}}
void ReadDistribution::observed(fragmentP frag){ //{{{
   ...
   long double Iexp = (gotExpression)? 1.0/trExp->exp(tid) : 1.0;
   // update lengths: //{{{
   DEBUG(message("   length update\n");)
   long double len,logLen;
   if(frag->paired){
      fragSeen ++;
      ...
      if(minFragLen>(long)len)minFragLen = len;
      logLen = log(len);
      logLengthSum += logLen;
...
void ReadDistribution::normalize(){ //{{{
   // length distribution: {{{
   long double newMu=0, newSigma=0;
   ...
   // }}}
   if(uniform) return;
   map<long,long double>::iterator mIt;
   long i,j,m,group,trLen,fragLen;
   long double Iexp,norm;
   ...
   for(m=0;m<M;m++){
      ...
      for( mIt=trFragSeen3[m].begin(); mIt != trFragSeen3[m].end(); mIt++){
         ...
         for(i=0;i<trNumberOfBins;i++){
            ...
            if(i*trLen/trNumberOfBins < fragLen){
               posProb[uniformM_3][group][trNumberOfBins -1 -i]+= Iexp * ((long double)(i+1)*trLen / trNumberOfBins - fragLen + 1);
            }else{
               posProb[uniformM_3][group][trNumberOfBins -1 -i]+= Iexp * ((long double)trLen / trNumberOfBins);
            }
         }
      }  
      ...
      for( mIt=trFragSeen5[m].begin(); mIt != trFragSeen5[m].end(); mIt++){
         ...
         for(i=0;i<trNumberOfBins;i++){
            ...
            if((i+1)*trLen/trNumberOfBins < fragLen)continue;
            if(i*trLen/trNumberOfBins < fragLen){
               posProb[uniformM_5][group][i]+= Iexp * ((long double)(i+1)*trLen / trNumberOfBins - fragLen + 1);
            }else{
               posProb[uniformM_5][group][i]+= Iexp * ((long double)trLen / trNumberOfBins);
            }
         }
      }  
   }// }}}
   ...
   long double IexpSum5,IexpSum3;
   map<long,long double>::reverse_iterator mItR;
   long p;
   for(m=0;m<M;m++){
      if(verbose)progressLogRD(m,M);
      trLen = trInf->L(m);
...
pair<long double,long double> ReadDistribution::getSequenceProb(bam1_t *samA){//{{{
   if(! samA) return pair<long double, long double>(1,1);
   //return pair<long double, long double>(1,0.00000001);
   long double prob=1,lowProb=1,probMis;
   ...
   for(i=len-1;i>=0;i--){
      probMis = pow((long double)10.0,((long double) qualP[i])/-10.0 );
   ...
   //message("%lf %lf\n",prob,lowProb);
   return pair<long double, long double>(prob,lowProb);
}//}}}
void ReadDistribution::getP(fragmentP frag,long double &prob,long double &probNoise){ //{{{
   long double P = 1;
   prob = 0;
   probNoise = 0;
   pair<long double, long double> pSeq1,pSeq2;
   ...
   long double trLen = trInf->L(tid),len;
   // }}}
   if(frag->paired){
...
void ReadDistribution::updatePosBias(long pos, biasT bias, long tid, long double Iexp){ //{{{
...
void ReadDistribution::updateSeqBias(long pos, biasT bias, long tid, long double Iexp){ //{{{
   if(Iexp==0)return;
...
long double ReadDistribution::getPosBias(long pos, readT read, long tid){ //{{{
   long group, rel, trLen;
   if(read == mate_5)pos--;
   trLen = trInf->L(tid);
   // transcript too short:
...
long double ReadDistribution::getSeqBias(long pos, readT read, long tid){ //{{{
...
long double ReadDistribution::getWeightNorm(long len, readT read, long tid){ //{{{
   if(len == 0)return 1;
   if(weightNorms[read][tid].count(len) == 0){
      long trLen = trInf->L(tid),pos;
      long double norm = 0,w;
...
long double ReadDistribution::getLengthP(long double len){//{{{
   return 1./(len*lSigma*2.5066282746)*exp(-pow(log(len) - lMu, (long double)2.0)/(2 * pow(lSigma, (long double)2)));
}//}}}
long double ReadDistribution::getLengthNorm(long double trLen){//{{{
...
vector<long double> ReadDistribution::getEffectiveLengths(){ //{{{
   vector<long double> effL(M,0);
   long m,len,trLen,pos;
   long double eL, cdfNorm,lenP, wNorm;
   MyTimer timer;
   timer.start();
...
// always computing the effective length using fragLen only
      if(uniform){
         ...
         vector<long double> posBias5(trLen),posBias3(trLen);
...
long double VlmmNode::getPsum(char b) {//{{{
...
void VlmmNode::update(long double Iexp, char b, char bp, char bpp) {//{{{
   long double expDiv = 1.0;
...
long double VlmmNode::getP(char b, char bp, char bpp) {//{{{
   if(base2int(b) == -1)return 1.0/4.0;
   long double probDiv = 1.0;
   ...
   for(i=0;i<samC->n_cigar;i++){
      ...
      for(j=0;j<opN;j++){
         switch(bam1_cigar(samA)[i]&BAM_CIGAR_MASK){
            case 0:
            case 7: //M,=
               prob *= 1 - pow((long double)10.0,((long double) qualP[bi])/-10.0 );
               if(misses>0){
                  lowProb *= pow((long double)10.0,((long double) qualP[bi])/-10.0 );
                  misses--;
               }else{
                  lowProb *= 1 - pow((long double)10.0,((long double) qualP[bi])/-10.0 );
               }
               break;
            case 1: case 2: case 3: case 4: case 5: case 6: case 8: 
            //I,D,N,S,H,P,X
               prob *= pow((long double)10.0,((long double) qualP[bi])/-10.0 );
               lowProb *= pow((long double)10.0,((long double) qualP[bi])/-10.0 );
               break;
         }
         bi+=biMove;
