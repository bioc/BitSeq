...
//   distributionParameters *dir;
   const vector<long> *alignI;
   const vector<TagAlignment> *alignments;
   const vector<long double> *isoformLengths;
   ...
   Sampler();
   virtual ~Sampler();
   void init(long m, long samplesTotal, long samplesOut, long Nmap, long Nunmap, const vector<long> &alignI, const vector<TagAlignment> &alignments, const distributionParameters &betaPar, const distributionParameters &dirPar, long seed);
   ...
   void saveSamples(ofstream *outFile, const vector<long double> *isoformLengths, outputType saveType, double norm = 0);
   void noSave();
