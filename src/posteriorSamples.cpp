...
bool PosteriorSamples::open(string fileName){//{{{
   ...
   if(!samplesF.is_open()){
      errmessage("PosterioSamples: File open failed: \%s\\n",(fileName).c_str());
      failed=true;
      return false;
   }
   return true;
...
bool PosteriorSamples::initSet(long &m,long &n, string fileName){//{{{
   ...
   if(!fh.samplesHeader(N,M,transposed,areLogged)){
      errmessage("PosteriorSamples: File header reading failed.\n");
      failed=true;
      return false;
   }
   n=N;
...
bool PosteriorSamples::read(){//{{{
   ...
   if(transposed){
      ...
      if(N*M > PS_maxStoredSamples){
         errmessage("PosteriorSamples: Too many samples to store,use trasposed file.\n");
         failed=true;
         return false;
      }
   ...
   if(transposed){
      ...
      for(i=0;(i<N)&&(samplesF.good());i++){
         ...
         if(samplesF.fail()){
            ...
            else errmessage("PosteriorSamples: Unknown value: %s in [tr:%ld,pos:%ld]\n",(str).c_str(),tr,i);
            good=false;
         }
      }
      if(i!=N){
         good=false;
         errmessage("PosteriorSamples: Reading failed at position:  [tr:%ld,pos:%ld]\n",tr,i);
      }
   }else{
...
bool Conditions::init(long &c,long &m,long &n,string trFileName, vector<string> filesGot){//{{{
   ...
   // if there is no transcript join file, the we have to make sure that Ms are the same
      if(sameMs){
         ...
         errmessage("Conditions: Different number of transcripts and missing transcript-join file\n");
         return false;
      }  
   }else{
      FileHeader fh(&trFile);
      if((!fh.transcriptsHeader(M,colN))||(M==0)||(colN<CN+1)){
         errmessage("Conditions: Wrong transcript join descriptor file - m: %ld colN: %ld\n",M,colN);
         return false;
      }
      m=M;
...
bool Conditions::getTranscript(long cond, long tr, vector<double> &trSamples){//{{{
   ...
   if(cond>=CN){
      errmessage("Conditions: Wrong condition request.\n");
      return false;
   }
   if(tr>=M){
      errmessage("Conitions: Wrong transcript request.\n");
      return false;
   }
...
bool Conditions::getTranscript(long cond, long tr, vector<double> &trSamples, long samplesN){//{{{
   ...
   if(samplesN > Ns[cond]){
      errmessage("Conitions: Wrong not enough samples.\n");
      return false;
   }
   if(samplesN <1){
      errmessage("Conitions: Wrong number of samples.\n");
      return false;
   }
   if(mapping)tr=trMap[tr][cond];
   if(samplesN != Ns[cond]){
